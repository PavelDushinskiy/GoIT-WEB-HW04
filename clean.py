...
def sort_files(folder_path):
    ...
    for file_path in file_paths:
        ...
        if os.path.exists(f'{root_folder}\\{inv_dict_ext.get(extension, "other")}\\{file_name}'):
            file_name += '.copy'

        try:
            th = Thread(target=os.rename,
                        args=(file_path, f'{root_folder}\\{inv_dict_ext.get(extension,"other")}\\{file_name}'))
            th.start()
            threads.append(th)
